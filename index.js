...
function updateBalance() {
	...
	if (balance > 0) {
		availableBalanceElement.innerText = `Możesz jeszcze wydać ${balance.toFixed(2)} PLN`;
	} else if (balance === 0) {
		availableBalanceElement.innerText = 'Bilans wynosi zero';
	} else {
		availableBalanceElement.innerText = `Bilans jest ujemny. Jesteś na minusie ${Math.abs(balance).toFixed(2)} PLN`;
	}
}
...
function editIncome(index) {
	...
	if (newName !== null && newName.trim() !== '' && !isNaN(newAmount) && newAmount > 0) {
...
function updateBalance() {
	...
	totalIncomeElement.innerText = `Całkowity przychód: ${totalIncome.toFixed(2)} PLN`;
	totalExpenseElement.innerText = `Całkowity wydatek: ${totalExpense.toFixed(2)} PLN`;

	if (balance > 0) {
		availableBalanceElement.innerText = `Dostępna kwota: ${balance.toFixed(2)} PLN`;
	} else if (balance === 0) {
		availableBalanceElement.innerText = 'Dostępna kwota: 0.00 PLN';
	} else {
		availableBalanceElement.innerText = `Dostępna kwota: -${Math.abs(balance).toFixed(2)} PLN`;
	}
}
